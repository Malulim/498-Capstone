 ...
 * in_flight_order_count would exceed max_in_flight <= ORDER_TABLE_SIZE. */
int insert_order_into_table(OrderTable *table, unsigned int order_id, Decision decision);

/* Sweep the table for IN_FLIGHT orders older than FILL_DELAY_SEC, move their
 * shares out of in_flight_net_shares into *settled_position_shares, and free
 * their slot back to EMPTY. Returns the number of ORDERS filled (not shares),
 * so the caller can log the position change. */
