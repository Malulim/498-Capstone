...
RiskReject risk_guard_check(const RiskParams *risk_params,
    ...
    long long exposure = (long long)settled_position_shares + in_flight_net_shares;
    ...
    // RISK_IN_FLIGHT）。in_flight_net_shares 已在上面的 position 检查里用上了。
    (void)in_flight_order_count;
    return RISK_OK;
}
